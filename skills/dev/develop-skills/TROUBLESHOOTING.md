...
```bash
cat .pi/skills/skill-rules.json | jq .
```
...
```bash
ls .pi/hooks/state/
cat .pi/hooks/state/skills-used-{session-id}.json
```
...
```bash
rm .pi/hooks/state/skills-used-{session-id}.json
```
...
```bash
cat <<'EOF' | npx tsx .pi/hooks/skill-verification-guard.ts 2>&1
{
  "session_id": "debug",
  "tool_name": "Edit",
...
```bash
cat .pi/settings.json | jq '.hooks.UserPromptSubmit'
cat .pi/settings.json | jq '.hooks.PreToolUse'
```
...
```bash
head -1 .pi/hooks/skill-activation-prompt.sh
```
...
```bash
# UserPromptSubmit
time echo '{"prompt":"test"}' | npx tsx .pi/hooks/skill-activation-prompt.ts

# PreToolUse
time cat <<'EOF' | npx tsx .pi/hooks/skill-verification-guard.ts
