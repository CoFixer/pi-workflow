...
1. **Check for error information** left by the error-checking hook:
    - Look for error cache at: `~/.pi/tsc-cache/[session_id]/last-errors.txt`
    - Check affected repos at: `~/.pi/tsc-cache/[session_id]/affected-repos.txt`
    - Get TSC commands at: `~/.pi/tsc-cache/[session_id]/tsc-commands.txt`
...
cat ~/.pi/tsc-cache/*/last-errors.txt

# 2. Check which TSC commands to use
cat ~/.pi/tsc-cache/*/tsc-commands.txt
...
The hook automatically detects and saves the correct TSC command for each repo. Always check `~/.pi/tsc-cache/*/tsc-commands.txt` to see which command to use for verification.
