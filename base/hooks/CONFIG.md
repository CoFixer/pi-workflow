...
### 1. Register Hooks in .pi/settings.json

Create or update `.pi/settings.json` in your project root:
        ...
        "UserPromptSubmit": [
            {
                "hooks": [
                    {
                        "type": "command",
                        "command": "$CLAUDE_PROJECT_DIR/.pi/hooks/skill-activation-prompt.sh"
                    }
                ]
            }
        ],
        "PostToolUse": [
            {
                ...
                "hooks": [
                    {
                        "type": "command",
                        "command": "$CLAUDE_PROJECT_DIR/.pi/hooks/post-tool-use-tracker.sh"
                    }
                ]
            }
        ],
        "Stop": [
            {
                "hooks": [
                    {
                        "type": "command",
                        "command": "$CLAUDE_PROJECT_DIR/.pi/hooks/stop-prettier-formatter.sh"
                    },
                    {
                        "type": "command",
                        "command": "$CLAUDE_PROJECT_DIR/.pi/hooks/stop-build-check-enhanced.sh"
                    },
                    {
                        "type": "command",
                        "command": "$CLAUDE_PROJECT_DIR/.pi/hooks/error-handling-reminder.sh"
                    }
                ]
            }
        ]
...
Edit `.pi/hooks/post-tool-use-tracker.sh`, function `detect_repo()`:
...
Edit `.pi/hooks/post-tool-use-tracker.sh`, function `get_build_command()`:
...
Edit `.pi/hooks/post-tool-use-tracker.sh`, function `get_tsc_command()`:
...
Edit `.pi/hooks/stop-prettier-formatter.sh`, function `get_prettier_config()`:
...
Configure file category detection in `.pi/hooks/error-handling-reminder.ts`:
...
Edit `.pi/hooks/stop-build-check-enhanced.sh`:
...
```
$CLAUDE_PROJECT_DIR/.pi/tsc-cache/[session_id]/
```
...
rm -rf $CLAUDE_PROJECT_DIR/.pi/tsc-cache/*

# Remove specific session
rm -rf $CLAUDE_PROJECT_DIR/.pi/tsc-cache/[session-id]
```
...
1. **Check registration:** Verify hook is in `.pi/settings.json`
2. **Check permissions:** Run `chmod +x .pi/hooks/*.sh`
3. **Check path:** Ensure `$CLAUDE_PROJECT_DIR` is set correctly
4. **Check TypeScript:** Run `cd .pi/hooks && npx tsc` to check for errors
        ...
        "PreToolUse": [
            {
                ...
                "hooks": [
                    {
                        "type": "command",
                        "command": "$CLAUDE_PROJECT_DIR/.pi/hooks/my-custom-bash-guard.sh"
                    }
                ]
            }
        ]
...
4. **Version control** - Commit `.pi/` directory to git
